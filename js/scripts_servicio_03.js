...
$("#contactForm_xs_01").validator().on("submit", function (event) {
    ...
    //Opcion de tipo empresa
    if ($('#tipo_empresa_xs_01').val() === '¿Quién solicita la asesoría?'){
        submitMSG_empresa_xs_01(false, "Seleccione una opción.");
    }
    else {
        event.preventDefault();
        submitMSG_empresa_xs_01(true, "");
        valido_select_01 = 1;
    }
    ...
    if ((valido_form_xs)&&(valido_select_01)&&(valido_check_xs) === 1) {
        submitForm_xs_01();
...
function submitForm_xs_01(){
    ...
    var tipo_empresa_xs_01 = ($('select[id=tipo_empresa_xs_01]').val());
    var telephone_xs_01 = $("#telephone_xs_01").val();


    $.ajax({
        type: "POST",
        url: "php/process_xs_04.php",
        data: "first_name_xs_01=" + first_name_xs_01 + "&email_xs_01=" + email_xs_01 + "&tipo_empresa_xs_01=" + tipo_empresa_xs_01 + "&telephone_xs_01=" + telephone_xs_01,
        success : function(text){
            if (text == "success"){
                formSuccess_xs_01();
                window.location.href = "agradecimiento_seguridad.html";
            } else {
...
function submitMSG_empresa_xs_01(valid, msg){
    if(valid){
        var msgClasses = "h4 text-center text-warning";
    } else {
        var msgClasses = "h4 text-center text-danger";
    }
    $("#msgSubmit_empresa_xs_01").removeClass().addClass(msgClasses).text(msg);
}
...
$("#contactForm_web_01").validator().on("submit", function (event) {
    ...
    //Opcion de tipo empresa
    if ($('#tipo_empresa_web_01').val() === '¿Quién solicita la asesoría?'){
        submitMSG_empresa_web_01(false, "Seleccione una opción.");
    }
    else {
        event.preventDefault();
        submitMSG_empresa_web_01(true, "");
    ...
    if ((valido_form)&&(valido_select_web_01)&&(valido_check) === 1) {
        submitForm_web_01();
...
function submitForm_web_01(){
    ...
    var tipo_empresa_web_01 = ($('select[id=tipo_empresa_web_01]').val());
    var telephone_web_01 = $("#telephone_web_01").val();

    $.ajax({
        type: "POST",
        url: "php/process_web_04.php",
        data: "first_name_web_01=" + first_name_web_01 + "&email_web_01=" + email_web_01 + "&tipo_empresa_web_01=" + tipo_empresa_web_01 + "&telephone_web_01=" + telephone_web_01,
        success : function(text){
            if (text == "success"){
                formSuccess_web_01();
                window.location.href = "agradecimiento_seguridad.html";
...
function submitMSG_empresa_web_01(valid, msg){
    if(valid){
        var msgClasses = "h4 text-center text-warning";
    } else {
        var msgClasses = "h4 text-center text-danger";
    }
    $("#msgSubmit_empresa_web_01").removeClass().addClass(msgClasses).text(msg);
}
